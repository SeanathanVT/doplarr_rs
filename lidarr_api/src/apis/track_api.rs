...
/// struct for typed errors of method [`api_v1_track_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ApiV1TrackGetError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`api_v1_track_id_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ApiV1TrackIdGetError {
    UnknownValue(serde_json::Value),
}

pub async fn api_v1_track_get(
    configuration: &configuration::Configuration,
    artist_id: Option<i32>,
    album_id: Option<i32>,
    album_release_id: Option<i32>,
    track_ids: Option<Vec<i32>>,
) -> Result<Vec<models::TrackResource>, Error<ApiV1TrackGetError>> {
    ...
    if let Some(ref param_value) = p_query_track_ids {
        req_builder = match "multi" {
            "multi" => req_builder.query(
                &param_value
                    .into_iter()
                    .map(|p| ("trackIds".to_owned(), p.to_string()))
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ApiV1TrackGetError> = serde_json::from_str(&content).ok();
...
pub async fn api_v1_track_id_get(
    configuration: &configuration::Configuration,
    id: i32,
) -> Result<models::TrackResource, Error<ApiV1TrackIdGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ApiV1TrackIdGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
