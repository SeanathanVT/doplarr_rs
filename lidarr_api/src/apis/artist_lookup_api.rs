...
/// struct for typed errors of method [`api_v1_artist_lookup_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ApiV1ArtistLookupGetError {
    UnknownValue(serde_json::Value),
}

pub async fn api_v1_artist_lookup_get(
    configuration: &configuration::Configuration,
    term: Option<&str>,
) -> Result<Vec<models::ArtistResource>, Error<ApiV1ArtistLookupGetError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ApiV1ArtistLookupGetError> = serde_json::from_str(&content).ok();
