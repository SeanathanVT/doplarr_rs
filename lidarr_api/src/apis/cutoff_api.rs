...
/// struct for typed errors of method [`api_v1_wanted_cutoff_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ApiV1WantedCutoffGetError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`api_v1_wanted_cutoff_id_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ApiV1WantedCutoffIdGetError {
    UnknownValue(serde_json::Value),
}

pub async fn api_v1_wanted_cutoff_get(
...
) -> Result<models::AlbumResourcePagingResource, Error<ApiV1WantedCutoffGetError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ApiV1WantedCutoffGetError> = serde_json::from_str(&content).ok();
...
pub async fn api_v1_wanted_cutoff_id_get(
    configuration: &configuration::Configuration,
    id: i32,
) -> Result<models::AlbumResource, Error<ApiV1WantedCutoffIdGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ApiV1WantedCutoffIdGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
