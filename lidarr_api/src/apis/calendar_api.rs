...
/// struct for typed errors of method [`api_v1_calendar_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ApiV1CalendarGetError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`api_v1_calendar_id_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ApiV1CalendarIdGetError {
    UnknownValue(serde_json::Value),
}

pub async fn api_v1_calendar_get(
    ...
    tags: Option<&str>,
) -> Result<Vec<models::AlbumResource>, Error<ApiV1CalendarGetError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ApiV1CalendarGetError> = serde_json::from_str(&content).ok();
...
pub async fn api_v1_calendar_id_get(
    configuration: &configuration::Configuration,
    id: i32,
) -> Result<models::AlbumResource, Error<ApiV1CalendarIdGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ApiV1CalendarIdGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
